...
**BoAT-Edge** 是摩联科技推出的一套面向物联网的区块链**端侧**应用框架，**它帮助物联网设备实现可信数据上链，赋能**数据从数据源头上传到云端以及后续物联网数据开放和交换下的全生命周期可信，是**BoAT家族各关键**组件的*重要组成部分*~~统称~~。**BoAT**全称为**B**lockchain **o**f **A**I **T**hing，***Edge特指***~~X泛指~~本产品在BoAT产品族中涵盖区块链端侧应用的各相关组件，包括嵌入式区块链客户端过包含的基础库和特定功能库以及配套工具。物联网设备可以通过灵活调用该套组件访问各类区块链服务。***~~BoAT-X所描绘的美好愿景是打造一艘船BoAT，从物联网设备这一数据的源头出发，承载着可信的物联网数据驶向千行百业的数据湖。~~***

- **BoAT-Edge~~BoAT-Infra-Arch~~** 是物联网端侧应用框架的总称，目前采用BoAT-Infra-Arch基础架构作为嵌入式区块链应用开发框架，适用于开发各种类型的区块链客户端，客户端也称之为物联网设备钱包。  
...
BoAT-Edge~~X~~产品族提供了一整套基础组件，利用区块链技术改造物联网端侧设备，在物联网设备端最靠近数据源头的位置建立信任锚点。BoAT-X可以从数据采集源头保证数据可信，来提高物联网数据价值，打造物联网可信数字底座。
