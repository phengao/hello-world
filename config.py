...
class ConfigContentGen():
    ...
    def gen_includepath(self):
        ...
        for curlib in self.usinglibs.split("\n"):
            ...
            if curlib == '':
                continue
            self.config_content += 'include $(BOAT_BASE_DIR)/' + curlib + '/include/includePath.conf\n'
    ...
    def gen_cJSON(self):
        ...
        self.config_content += '    BOAT_INCLUDE += -I$(BOAT_SDK_DIR)/third-party/cJSON\n'
        self.config_content += 'endif\n'
        self.config_content += '\n'
    ...
    # .PHONY
    def gen_PHONY(self):
        self.config_content += '.PHONY: all boatlibs createdir boatwalletlib vendorlib demo tests clean cleanboatwallet cleanvendor cleantests\n'
        self.config_content += '\n'
    
    # all:
    def gen_all(self):
        self.config_content += '#all: boatlibs demo tests\n'
        self.config_content += 'all: boatlibs\n'
        self.config_content += '\n'
    ...
    def gen_targets(self):
        self.config_content += 'boatlibs: createdir boatwalletlib vendorlib\n'
        self.config_content += '\n'
    ...
    def testgitfatal(self,subp):
        for line in subp.stdout:
            #print (line) 
            strline = line.decode()
            offset = strline.find('fatal:')
            print('stdout:' + strline + '\n')
            if offset >= 0:
                print('stdout fatal ' + str(offset) +'\n')
                return True
            else:
                print('stdout no fatal ' + str(offset) +'\n')
                return False
        ...
        for line in subp.stderr:
            ...
            if offset >= 0:
                print('stderr fatal ' + str(offset) +'\n')
                return True
            else:
                print('stderr no fatal ' + str(offset) +'\n')
                return True # in stderr, means error

    def testCode(self):
        download_path = os.path.join('v223')
        print(download_path)
        # pull code 
    
        #subp = subprocess.Popen("git clone -b BoAT-X-v2.2.3 https://github.com/aitos-io/BoAT-X-Framework.git", shell=False, stdout=subprocess.PIPE)
        #for line in subp.stdout:
        #    print (line) 
    
        be = input(infoBoatEngine)
        ...
        if (be == '' ):
            ...
            print('key:' + be + ' is invalid\n')
        try:
            assert not be == ''
        
        except AssertionError as err:
            print('assert not error');
        else:
            print(' no error')
    ...
    def checkRepo(self,reponame):
        isExists = os.path.exists(reponame) # If the folder exists, the warehouse is considered to have been downloaded
        if not isExists:
            self.clonerepo(reponame)
        
        # pull the newest code?
        else:
            #the folder is existed , record the repo name
            self.usinglibs += reponame+'\n'
            needpull = self.getEnterAsNo('pull the repository')
            if needpull:
                os.chdir( './'+reponame )
...
def main():
    ...
    # Is the BoAT-SupportLayer directory existed?
    configContent_obj.checkRepo("BoAT-SupportLayer")

    # Add another library
    while(1):
        othrerrepo = input('Input another repository name:')
        if(othrerrepo == ''):
            norepo = configContent_obj.getEnterAsYes('Do not add other libraries anymore?')
            if norepo:
                break
        configContent_obj.checkRepo(othrerrepo)
